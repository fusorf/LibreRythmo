...
- **Import / export** — DETX (Joker / Cappella), SRT / ASS / VTT in (corrected-subtitle re-import for SRT), DETX and work-document PDFs out, MP4 composite with GPU encoding (NVENC / QuickSync / AMF, x264 fallback), an output frame-rate dropdown, and a choice of tracks, scenes and audio track.
- **Voice recording** — record takes right on the scrolling band (input meter, pre-roll), keep several takes per line and pick the keeper, preview and monitor them in sync during playback; retained takes are mixed into the MP4 export. Takes are portable sidecar files next to the project (`takes/`). A **capture device selector** (WASAPI, DirectShow, ASIO) lives in Settings.
- **Voice removal** — remove the voices from an audio track (produces an instrumental added as a new track) with a light local engine (MDX-Net ONNX). Optional, installed on demand — nothing bundled.
...
- **Automatic transcription** — bootstrap the text from the audio (multi-speaker: it detects the voices and creates one character each); the engine and models install on demand — nothing is bundled.
- **Settings** — a Settings window (View menu) with the capture device selector and a model manager: installing/removing models and engines is entirely optional and done from the interface.
